...
def _get_relevant_linkedin_post_images():
    ...
    results = []
    print(
        f"Looking for files in '{bucket_name}/{folder}' starting with '{starts_with}'..."
    )
...
def _linkedin_post_generator_function(
    input_text: str, aspect_ratio: str, tool_context: ToolContext
):
    """
    ...
    for uri in reference_images_uri:
        image_parts.append(types.Part.from_uri(file_uri=uri, mime_type="image/png"))
        if (
            len(image_parts) == 3
        ):  # We can only pass upto 3 images for gemini-2.5-image-preview
            break
    ...
    destination_blob_with_text = (
        f"linkedin_posts/{timestamp}_{uuid.uuid4().hex[:8]}_with_text.png"
    )
    ...
    generated_image_without_text = _remove_text_from_generated_image(
        gcs_uri_with_text=linkedin_post_gcs_uri
    )

    destination_blob_without_text = (
        f"linkedin_posts/{timestamp}_{uuid.uuid4().hex[:8]}_without_text.png"
    )
    blob = bucket.blob(destination_blob_without_text)
    blob.upload_from_string(data=generated_image_without_text, content_type="image/png")
