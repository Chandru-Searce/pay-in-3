...
def _edit_linkedin_post(user_prompt: str, tool_context: ToolContext, aspect_ratio: str):
    ...
    destination_blob_with_text = (
        f"linkedin_posts/{timestamp}_{uuid.uuid4().hex[:8]}_with_text_(edited).png"
    )
    blob = bucket.blob(destination_blob_with_text)

    blob.upload_from_string(data=generated_image, content_type="image/png")

    linkedin_post_uri_edited_with_text = (
        f"gs://{bucket_name}/{destination_blob_with_text}"
    )

    edited_image_without_text = _remove_text_from_generated_image(
        gcs_uri_with_text=linkedin_post_uri_edited_with_text
    )

    destination_blob_without_text = (
        f"linkedin_posts/{timestamp}_{uuid.uuid4().hex[:8]}_without_text_(edited).png"
    )
    blob = bucket.blob(destination_blob_without_text)
    blob.upload_from_string(data=edited_image_without_text, content_type="image/png")
