...
class Shot(BaseModel):
    ...
    subject: str = Field(
        ..., description="Characters and key objects in flat 2D brand style"
    )
    action: str = Field(
        ..., description="What the subject is doing, include motion style and easing"
    )
    style: str = Field(
        ..., description="Flat 2.5D vector illustration, Alegria or Corporate Memphis"
    )
    camera_positioning_and_motion: str = Field(
        ..., description="Camera movement description"
    )
    composition: str = Field(..., description="Framing and layout description")
    focus_and_lens_effects: str = Field(
        ...,
        description="Lens/focus effects; use Soft Aqua Marine glow only for key highlights",
    )
    ambiance: str = Field(
        ...,
        description="Mood using brand colors (Gulf Blue, Lovely Purple, Aqua Marine, etc.)",
    )
...
def _script_generator_func(
    ad_campaign_objective: str, output_file: str = VIDEO_SCRIPT_FILE
):
    """
    Generate ad video script JSON from user objective, validate with Pydantic,
    and save to a JSON file.
    """
    user_objective = Content(role="user", parts=[Part(text=ad_campaign_objective)])
    ...
    return {"status": "Successfully ad video script has been generated"}
