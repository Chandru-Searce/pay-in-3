...
def _generate_prompt_for_initial_frame():
    ...
    response = gemini_client.models.generate_content(
        ...
        contents=types.Content(
            role="user",
            parts=[
                types.Part(text="Here is the initial scene\n\n" + str(initial_scene))
            ],
        ),
...
def _generate_initial_frame():
    ...
    input_text = types.Part.from_text(text=initial_frame_generation_prompt)

    contents = types.Content(
        role="user", parts=[frame_1, frame_2, visual_reference, input_text]
    )
    ...
    first_frame = types.Part.from_bytes(data=image_bytes, mime_type="image/png")
...
def _get_last_frame_from_uri(uri: str) -> types.Part:
    ...
    try:
        ...
        # Wrap as Part object
        last_frame_image = types.Part.from_bytes(
            data=buffer.tobytes(), mime_type="image/png"
        )
...
def video_generation_func():
    ...
    for i, scene in enumerate(scenes):
        print(
            f"Generating video for scene {i + 1}/{len(scenes)}: {scene['scene_title']}"
        )
    ...
    final_clip.write_videofile(
        stitched_temp_file.name, codec="libx264", audio_codec="aac"
    )
    ...
    blob.upload_from_string(data=video_bytes, content_type="video/mp4")
