...
def _edit_ad_campaign_post(user_prompt: str, tool_context: ToolContext, aspect_ratio: str):
    """
    ...
    latest_ad_campaign_post_uri = tool_context.state.get("latest_ad_campaign_post_uri_with_text")

    latest_ad_campaign_post = types.Part.from_uri(
        file_uri=latest_ad_campaign_post_uri,
        mime_type="image/png"
    )
    ...
    destination_blob_with_text = f"ad_campaigns/{timestamp}_{uuid.uuid4().hex[:8]}_with_text(edited).png"
    ...
    destination_blob_without_text = f"ad_campaigns/{timestamp}_{uuid.uuid4().hex[:8]}_without_text(edited).png"
